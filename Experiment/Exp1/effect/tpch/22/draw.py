...
def draw(data1, data2, data3, data4):
    ...
    plt.plot(num, time4, marker=marker[3], color='#87CEFA', label='Embedding', lw=2, markersize=12)

    ax = plt.gca()
    ax.xaxis.set_major_locator(MultipleLocator(10))
    ax.yaxis.set_major_locator(MultipleLocator(0.1))

    plt.legend(fontsize=12, bbox_to_anchor=(0.35,0.4))
    ...
    plt.savefig('effect_22.png', dpi=300)
    plt.savefig('effect_22.pdf', dpi=300)
...
if __name__ == '__main__':
    ...
    data4 = clear_data(read_data('embedding.txt'))
